...
mean_changes_North_West <- mean(data$changes_total[data$Country_Cluster == "North West"], na.rm = TRUE)
print(mean_changes_North_West)

mean_changes_Central_West <- mean(data$changes_total[data$Country_Cluster == "Central West"], na.rm = TRUE)
print(mean_changes_Central_West)

mean_changes_Southern <- mean(data$changes_total[data$Country_Cluster == "Southern"], na.rm = TRUE)
print(mean_changes_Southern)

mean_changes_Central_East_Eastern <- mean(data$changes_total[data$Country_Cluster == "Central East and Eastern"], na.rm = TRUE)
print(mean_changes_Central_East_Eastern)

anova_result <- aov(changes_total ~ Country_Cluster, data = data)
...
mean_north_west <- round(mean(data$changes_total[data$Country_Cluster == "North West"], na.rm = TRUE), 3)
mean_central_west <- round(mean(data$changes_total[data$Country_Cluster == "Central West"], na.rm = TRUE), 3)
mean_southern <- round(mean(data$changes_total[data$Country_Cluster == "Southern"], na.rm = TRUE), 3)
mean_central_east <- round(mean(data$changes_total[data$Country_Cluster == "Central East and Eastern"], na.rm = TRUE), 3)
mean_total <- round(mean(data$changes_total, na.rm = TRUE), 3)  # Total Mean
...
total_overall <- sum(!is.na(data$changes_total))  # Total Count

# Standard deviation calculations
stddev_north_west <- round(sd(data$changes_total[data$Country_Cluster == "North West"], na.rm = TRUE), 3)
stddev_central_west <- round(sd(data$changes_total[data$Country_Cluster == "Central West"], na.rm = TRUE), 3)
stddev_southern <- round(sd(data$changes_total[data$Country_Cluster == "Southern"], na.rm = TRUE), 3)
stddev_central_east <- round(sd(data$changes_total[data$Country_Cluster == "Central East and Eastern"], na.rm = TRUE), 3)
stddev_total <- round(sd(data$changes_total, na.rm = TRUE), 3)  # Total Std Dev
...
# Save as a CSV file
#write.csv(anova_df, "anova_summary_country_clusters.csv", row.names = FALSE)
...
png("19_Mean_of_ratings_changes_country_cluster.png", width = 1200, height = 1000, res = 150)
