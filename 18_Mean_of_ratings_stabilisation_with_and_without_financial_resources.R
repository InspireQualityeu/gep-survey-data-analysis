...
mean_stabilisations_with_resources <- mean(data$stabilisations_total[data$Int_financial_res == 1], na.rm = TRUE)
print(mean_stabilisations_with_resources)

mean_stabilisations_without_resources <- mean(data$stabilisations_total[data$Int_financial_res == 2 | data$Int_financial_res ==3 ], na.rm = TRUE)
print(mean_stabilisations_without_resources)

# Perform the t-test
t_test_result <- t.test(
  data$stabilisations_total[data$Int_financial_res == 1], 
  data$stabilisations_total[data$Int_financial_res == 2 | data$Int_financial_res ==3 ], 
  var.equal = TRUE, na.rm = TRUE
)

# Extract relevant statistics with readable formatting
mean_stabilisations_with_resources <- round(mean(data$stabilisations_total[data$Int_financial_res == 1], na.rm = TRUE), 3)
mean_stabilisations_without_resources <- round(mean(data$stabilisations_total[data$Int_financial_res == 2 | data$Int_financial_res ==3], na.rm = TRUE), 3)

total_obs_stabilisations_with_resources <- sum(data$Int_financial_res == 1, na.rm = TRUE)  # Count for GEP=1
total_obs_stabilisations_without_resources <- sum(data$Int_financial_res == 2 | data$Int_financial_res ==3 , na.rm = TRUE)  # Count for GEP=0
...
t_test_df <- data.frame(
  ...
  Value = c(mean_stabilisations_with_resources, mean_stabilisations_without_resources, total_obs_stabilisations_with_resources, total_obs_stabilisations_without_resources, 
...
png("18_Mean_of_ratings_stabilisation_with_and_without_financial_resources.png", width = 1200, height = 800, res = 150)
