...
mean_stabilisations_total_gep <- mean(data$stabilisations_total[data$GEP_equivalent == 1], na.rm = TRUE)
print(mean_stabilisations_total_gep)

mean_stabilisations_total_no_gep <- mean(data$stabilisations_total[data$GEP_equivalent == 0], na.rm = TRUE)
print(mean_stabilisations_total_no_gep)


# Perform the t-test
t_test_result <- t.test(
  data$stabilisations_total[data$GEP_equivalent == 1], 
  data$stabilisations_total[data$GEP_equivalent == 0], 
  var.equal = TRUE, na.rm = TRUE
)

# Extract relevant statistics with readable formatting
mean_gep <- round(mean(data$stabilisations_total[data$GEP_equivalent == 1], na.rm = TRUE), 3)
mean_no_gep <- round(mean(data$stabilisations_total[data$GEP_equivalent == 0], na.rm = TRUE), 3)
...
png("15_Mean_of_ratings_stabilisation_with_and_without_GEP.png", width = 1200, height = 800, res = 150)
